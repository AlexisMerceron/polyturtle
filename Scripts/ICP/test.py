...
def test_icp():
    ...
    for i in range(num_tests):
        ...
        # Run ICP
        start = time.time()
        T, distances, iterations, r, t2 = icp.icp(B, A, tolerance=0.000001)
        total_time += time.time() - start
    ...
    ax.scatter([x[0] for x in A], [y[1] for y in A], [-z[2] for z in A], zdir='z', c='red')
    ax.scatter([x[0] for x in B], [y[1] for y in B], [-z[2] for z in B], zdir='z', c='blue')
    ax.scatter([x[0] for x in C], [y[1] for y in C], [-z[2] for z in C], zdir='z', c='green')
    plt.show()
